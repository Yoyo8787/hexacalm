...
角色不儲存位置、方向、路徑進度、跳躍姿勢、速度或相機模式。恢復世界時依 §13.2 的最大道路群組規則重新出生，恢復行走啟用狀態，速度回到 1×、視角回到 Builder View；無有效道路時移除角色。

開發期間不提供舊格式相容；持久化格式變更時更新版本，版本不符的資料依 §19 清除。
...
角色移動不持續寫入位置，僅在角色種類或行走啟用狀態改變時更新角色存檔。
...
建造介面與操作僅在 Builder View 開放；第一／第三人稱隱藏建造介面及可放置空格，保留角色、音訊、模式與視角控制。
...
角色操作不加入建造歷史；Undo／Redo 不還原角色種類、位置、速度或行走啟用狀態，也不復活已移除的角色。下列角色路徑更新僅用於適應還原後的道路。
...
第一／第三人稱鏡頭高度保持穩定，避免跟隨角色小跳上下震動。恢復世界時使用 Builder View，不儲存相機模式。
...
- 系統找出有效道路 Tile 數量最多的連通群組；同大小時隨機選一組，再從組內隨機選出生道路。
- 角色生成於該 Tile 上。
- 新加入角色預設啟用行走；恢復存檔時使用已儲存的行走啟用狀態。
- Listener 切換為 Character 模式。

孤立道路也可加入角色，無路時原地待機；已啟用行走的角色在道路補通後自動續行。角色位置仍有效時，不因其他群組變大而重新安置。
...
無連通方向時原地待機並保留行走啟用狀態；補通道路後自動續行。使用者手動停止後不自動續行。

分頁進入背景時暫停實際移動與腳步聲，保留行走啟用狀態；回到前景時繼續，不補跑背景期間距離。環境音與間隔叫聲維持播放，仍受音訊控制。

## 14.3 速度控制

使用者可調整角色移動速度。

速度以滑桿控制，預設 1×，範圍 0.5×–2×；1× 先以相鄰直道路格中心之間約兩秒為調整起點。
...
當 Add、Replace、Rotate、Remove、Undo、Redo 改變道路時：

- 系統重新檢查角色目前位置與可行方向。
- 重新建立可行路徑。
- 若角色所在位置仍有效，角色從目前位置繼續。
- 若角色目前道路位置已失效，依 §13.2 的最大道路群組規則重新安置，保留種類、速度與行走啟用狀態。
- 若世界中已無有效道路，移除角色。
...
1. 尋找其他有效道路 Tile。
2. 若存在，依最大道路群組規則重新安置，保留角色種類、速度、行走啟用狀態、Character Listener 與目前視角。
3. 若不存在，移除角色，Listener 回到 World 模式，視角回到 Builder View。
4. 之後重建道路或 Undo 還原道路，不自動重新加入已移除的角色。
...
Listener 與鏡頭無關；Builder、第一人稱與第三人稱均使用角色位置與朝向。初始有效距離以約三格為調整起點，近處較清楚、遠處逐漸淡出。移除角色後平順回到 World Listener。
...
已確認的細節與分階段驗收見 [角色規格與 Commit 計畫](./docs/character-spec.md)，定義：
